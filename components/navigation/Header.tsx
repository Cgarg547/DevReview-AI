...
const Plan: React.FC<{ reviews: number | undefined }> = ({ reviews }) => {
  ...
  if (hasProPlan) {
    return (
      <div className="flex shrink-0 items-center gap-2 rounded-full border border-yellow-700 bg-yellow-900/50 px-3 py-2 text-xs font-semibold text-yellow-300 sm:text-sm">
  ...
  return (
    <div className="flex min-w-0 items-center gap-2">
      <div className="hidden items-center gap-2 rounded-full bg-gray-700 px-3 py-2 text-xs sm:flex sm:text-sm">
        ...
        <span className="font-semibold text-white">{reviews ?? 0}</span>
      </div>
...
const Header = () => {
  ...
  return (
    <header className="sticky inset-x-0 top-0 z-50 h-[68px] w-full shrink-0 border-b border-gray-700 bg-gray-900/95 shadow-lg backdrop-blur-md">
      <div className="mx-auto flex h-full w-full max-w-7xl items-center justify-between gap-3 px-3 sm:px-6 lg:px-8">
        <Link
          href="/"
